...
parser.add_argument('-q', '--quiet', dest='quiet', action='store_true', help='Be quiet (=disable info logs).')
parser.add_argument('-c', '--use-cache', dest='useCache', action='store_true', help="Use caches containing enclosure information. Useful to speedup scripts. THIS SHOULD only be used after running without cache since the last hardware modification! Otherwise the results might be WRONG!")
parser.add_argument('-e', '--enable-id', dest='enableId', action='store_true', help="Enable the ident LED for the specified disks")
parser.add_argument('-d', '--disable-id', dest='disableId', action='store_true', help="Disable the ident LED for the specified disks")
...
def run(cmd, splitLinesBy='\n', check=False, errorLineFilter=None):
    ...
    if errorLineFilter:
        err = "\n".join(filter(lambda l : not errorLineFilter(l), err.split("\n")))
    if err:
...
class Enclosure():
    def __init__(self, sgName):
        self.sgName = sgName
        self.model = readFile(ScsiGenericSysDir + sgName + "/device/model", "UNKNOWN").strip()
        self.disks = None
    ...
    def findDisks(self):
        disklines = getFromCache(
                self.sgName + '_ses_j',
                lambda : run(
                    "sudo sg_ses  -j /dev/" + self.sgName + "| grep -E '^(\\[0,[0-9]+\\] *Element type: Array device slot| *SAS address:)' | grep -v 'SAS address: 0x0$' | grep -B1 'SAS address:' | grep -Eo '\\[.*\\]| 0x5.*' | sed '/\\[/{N;s/\\n//}' | grep -E '\\[[0-9]+,[0-9]+\\]'",
        ...
        if options.debug:
            debug("Parsing this output of sg_ses:", disklines);
        return [ Disk(d.split(" ")[1], self, d.split(" ")[0].split(',')[1][:-1]) for d in disklines if d ]

if __name__ == "__main__":
    run("mkdir -p ~/.cache")

    with shelve.open(os.path.expanduser("~/.cache/sas-disks-id.cache")) as cache:
        enclosures = list(filter(lambda x: x, run("grep -lF 13 %s*/device/type | grep -Eo sg[0-9]+" % ScsiGenericSysDir)))
        ...
        for dPath in options.disks:
            ...
            if not d in sdX2sas:
                error("%s does not seem to have a SAS address (see lsscsi -t) -- skipping!" % dPath)
                continue

            sasAddr = sdX2sas[d]
            if not sasAddr in sas2disk:
                error("Could not find %s(SAS:%s) within the enclosures -- skipping!" % (d, sasAddr))
                continue
            ...
            if options.disableId:
                ...
                run("sudo sg_ses --index=%s --clear=ident /dev/%s" % (disk.sesAddr, disk.enc.sgName))
            elif options.enableId:
                info("Turning ON ident LED for '%s'" % disk)
                run("sudo sg_ses --index=%s --set=ident /dev/%s" % (disk.sesAddr, disk.enc.sgName))
            else:
                print("%s is in slot %d in enclosure %s" % (disk, 1 + int(disk.sesAddr), disk.enc))
